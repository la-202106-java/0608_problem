...
public class MaterialCatalogDAO {
	...
	public MaterialCatalog findBySelect(String title, String author, String publisher, String publisher_date,
			int categoryCode)
			throws DAOException {
		String sql = "SELECT * FROM material_catalog where title = ? and category_code=? and authur=? and publisher=? and publication_date=?";
		try (Connection con = ConnectionFactory.createConnection();
				PreparedStatement st = con.prepareStatement(sql);) {
			// カテゴリの設定
			...
			st.setDate(5, Date.valueOf(publisher_date));
			try (ResultSet rs = st.executeQuery()) {
				// 結果の取得および表示
				if (rs.next()) {
					String isbn2 = rs.getString("isbn");
					...
					Date publicationDate = rs.getDate("publication_date");
					MaterialCatalog bean = new MaterialCatalog(isbn2, title1, categoryCode2,
							authur, publisher2, publicationDate);
					return bean;
				} else {
					return null; // 主キーに該当するレコードなし
				}
