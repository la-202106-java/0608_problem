...
import la.dao.ItemDAO6;

@WebServlet("/ItemServlet6")
public class ItemServlet6 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			...
			// モデルのDAOを生成
			ItemDAO6 dao = new ItemDAO6();
			// パラメータなしの場合は全レコード表示
			if (action == null || action.length() == 0) {
				List<ItemBean6> list = dao.findAll();
			...
			// sortはソート
				...
				List<ItemBean6> list = (List<ItemBean6>) session.getAttribute("items");
				if (list == null) {
					...
					list.sort(Comparator.comparing(ItemBean6::getPrice));
					session.setAttribute("items", list);
				} else {
					list.sort(Comparator.comparing(ItemBean6::getPrice).reversed());
					session.setAttribute("items", list);
				}
				gotoPage(request, response, "/items.jsp");
				// searchは検索
				...
				List<ItemBean6> list = dao.findByPrice(minprice, maxprice, name, category);
				// Listをリクエストスコープに入れてJSPへフォーワードする
